new Vue({
    ...
    methods: {
        ...
        startGetWeight:function(pic,name,price) {
            ...
            vm.getWeight(function(weight) {
                vm.calculateGoodsList.push({
                    ...
                    price:price,
                    weight:weight,
                    totalPrice:parseFloat(price*weight).toFixed(3)
                });
            });
        },
        /**
        ...
        deleteGoods:function(id) {
            vm.calculateGoodsList =  vm.calculateGoodsList.filter(function(value) {
                return value.id != id;
            });
        },
        /**
        ...
        getWeight: function (callback) {
            ...
            //             // 已经稳定
            //             callback(data.data.Weight.toFiexed(3));
            //         }
            //     }).catch(function (err) {
            //         vm.log(err);
            //     });
    ...
    computed:{
        totalMoney:function() {
            // console.log(this.calculateGoodsList);
            // return this.calculateGoodsList.reduce(function(pre,next) {
            //     console.log('pre='+(pre.totalPrice) + ',next='+(next.totalPrice));
            //     return (parseInt(pre.totalPrice) + parseInt(next.totalPrice));
            // },{totalPrice:0});
            return [1,2,3,5].reduce(function(pre,next) {
                return pre+next;
            },0);
        }
    },
