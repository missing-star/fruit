new Vue({
    ...
    methods: {
        ...
        watchScroll: function (ref) {
            var vm = this;
            vm.$nextTick(function () {
                vm.$refs[ref].addEventListener('scroll', function (e) {
                    var moreTop = document.querySelector('.no-more').getBoundingClientRect().top;
                    if (moreTop < document.documentElement.clientHeight && vm.loadMore) {
                        //开始加载数据
                        getNewsList(vm.currentTab, ++page);
                    }
                });
            });
        },
         ...
         * 获得订单支付二维码地址
         */
        getPayQrCode: function (orderNum) {
            var vm = this;
            // 获取二维码地址
